...
do
{
  ...
  @$result = Pg_Exec(
		"select oid, gnazev, bcislo01, lcteni, scteni from graf01 WHERE bcislo01='$co'");
  if (!$result):
    echo "There was an error by the SQL query!";
    break;
  endif;
  echo "<table border=1>";
  echo "<tr><td><center><small>sign name</small></center></td><td><center><small>ABZ number</small></center></td><td><center><small><b>logographic reading</b></small></center></td><td><center><small><b>syllabic reading</b></small></center></td></tr>";
  echo "<FORM id=form1 METHOD=\"get\" name=form1 ACTION=\"/utf/utf/signedit2.php\" ACCEPT-CHARSET=\"utf-8\" enctype=\"multipart/form-data\">";
  Pg_NumRows($result);
  $zaznam = Pg_Fetch_Array($result, $i);
	$oid=$zaznam["oid"];
//	include "./edit2.php";
  echo ("<tr><td><p class=vstup><small>".$zaznam["gnazev"]."</small></p></td><td><p class=vstup cols=90 rows=10><small>".$zaznam["bcislo01"]."</small></p></FONT></td></TR><TR>
<td><FONT FACE='Arial Unicode MS' SIZE=3><textarea class=vstup name=lcteni cols=90 rows=1>".$zaznam["lcteni"]."</textarea></FONT></td></TR><TR>
<td><FONT FACE='Arial Unicode MS' SIZE=3><textarea class=vstup id=q name=scteni cols=90 rows=3>".$zaznam["scteni"]."</textarea></FONT></td></tr>\n");
