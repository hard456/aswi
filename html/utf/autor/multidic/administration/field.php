  ...
  $navrat .= "<table>
      ...
      Zobrazeno $limit oborů od $od.  (Ve slovniku celkem $pocet_oboru)<br />
       ...
       <select name=\"order\">	
         ...
         <option value=\"field\">název</option>
       </select>
...
function insert_field($field) {
  require_once("./classes/db.php");
  $spojeni = new DB_Sql();
  $dotaz = "INSERT INTO field (field) 
                    VALUES ('$field')";
  $spojeni->query($dotaz);
  if ($spojeni->Errno != 0) {
    print_hlasku("Bohužel, obor '$field' se nepodařilo přidat.");
    return false;
  }
  print_hlasku ("Obor '$field' přidán...");
  return true;
}
//NOVE2504
...
function print_table_of_field($order = "IDfield", $od = 0, $limit = 30) {
  ...
  while ($spojeni->next_record()) {
    $navrat .= get_row_of_table($spojeni->Record, 1, 2);
  }
...
//NOVE 2504
function update_field($name, $field_id) {
  require_once("./classes/db.php");
  $spojeni = new DB_Sql();
