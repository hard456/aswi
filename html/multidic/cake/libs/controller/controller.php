	...
	function generateFieldNames($data = null, $doCreateOptions = true) {
		...
		foreach($objRegistryModel->_tableInfo->value as $tabl) {
			...
			if (isset($validationFields[$tabl['name']])) {
				///pr($validationFields[$tabl['name']]);
				//upraveno podle noveho valid. modelu
				foreach($validationFields[$tabl['name']] as $k=>$valArray) {
					if (VALID_NOT_EMPTY == $valArray[0]) {
						$fieldNames[$tabl['name']]['required'] = true;
						$fieldNames[$tabl['name']]['errorMsg'] = "Vyžadovaná položka";
					}
				}
			}
 ...
 */
	function cleanUpFields($modelName = null) {
		...
		foreach($this->{$modelName}->_tableInfo->value as $field) {
			if ('date' == $field['type'] && isset($this->params['data'][$modelName][$field['name'] . '_year'])) {
				...
				//bud to takhle nechat nebo kdyz neco z toho neni vyplneno, tak vymazat
				if ($this->controlDate($newDate) == true) {
					$this->params['data'][$modelName][$field['name']] = $newDate;
					$this->data[$modelName][$field['name']] = $newDate;
				}
				else {
					unset($this->params['data'][$modelName][$field['name']]);
					unset($this->data[$modelName][$field['name']]);
				}
	...
	/**
	 * Volano pro kontrolu data.
	 *
	 * Pokud OK, pak vraci true, jinak false.
	 * */
	function controlDate($date) {
		$pattern = '/(\d*)-(\d*)-(\d*)/i';
		$year = preg_replace($pattern, '$1', $date);
		$month = preg_replace($pattern, '$2', $date);
		$day = preg_replace($pattern, '$3', $date);
