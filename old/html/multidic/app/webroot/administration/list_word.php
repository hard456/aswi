...
?>
  ...
  <p>
    ...
    <a href="?nav_id=<?php echo $nav_id?>&language=all&contrains_source=all&contrains_lection=all<?php echo $pomocna?>">všechny</a>
  </p>
<?php
...
if (!Empty($action) && $action == "delete_word") {
  if (delete_word($word_id))
    print_hlasku("Slovíčko smazáno.");
  else
    print_hlasku("Slovíčko se nepodařilo smazat");
...
if (!Empty($action) && $action == "sort_direct") {
  set_sorting_value($word_id, $sorting_no);
}
//obsluha zmeny raditka
if (!Empty($action) && $action == "sort_word") {
  sort_word($word_id, $dir);
}
...
//vypis
if (!Empty($serad)) {
    print_all_in_dict(false, $language, $contrains_source, $contrains_lection, $pomocna, $contrains_user_id, $order, $od, $limit);
}
else {
  print_all_in_dict(false, $language, $contrains_source, $contrains_lection, $pomocna, $contrains_user_id);
}
