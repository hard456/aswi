...
// skládám podmínku;
  $prvni = 1;
  $query = "SELECT idbook, title, subtitle, volume_shortcut, number, place, year, signature FROM book WHERE ";
...
// --------
  ...
  {
   $query = "SELECT idbook, title, subtitle, volume_shortcut, number, place, year, signature FROM book ORDER BY title";
  }
  else
  {
...
// nactu, jake ma autor publikace
  ...
  if ($vysl_row3 != 0)
  {
   ...
   for ($i = 0; $i < $vysl_row3; $i++)
   {
	...
	$idbook_tmp=$idbook;
	$query4 = "SELECT idbook, title, subtitle, volume_shortcut, number, place, year, signature FROM book WHERE idbook = $idbook_tmp";
    ...
    if ($vysl_row4 != 0)
    {
     ...
     {
      List($idbook, $title, $subtitle, $volume_shortcut, $number, $place, $year, $signature) = Pg_Fetch_Row ($vysl4, $j, PGSQL_NUM);
// naèítám autory;
