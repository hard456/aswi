...
function insert_user($name, $surname, $city, $email, $nationality, $privileges, $nick, $password){
  require_once("./classes/db.php");
  $spojeni = new DB_Sql();
  //$NOW = Date("YmdHis");
  ...
  $spojeni->query($dotaz);
  if ($spojeni->Errno != 0) {
    print_hlasku("Bohužel, uživatele '$name $surname' se nepodařilo přidat.");
    return false;
  }
  print_hlasku ("Uživatel '$name $surname' přidán..");
  return true;
}
...
function update_user($name, $surname, $city, $email, $nationality, $privileges, $nick, $user_id) {
  require_once("./classes/db.php");
  $spojeni = new DB_Sql();
