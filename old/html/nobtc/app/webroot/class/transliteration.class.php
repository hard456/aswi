...
class OldTransliteration { // BEGIN class Transliteration
	// variables
	var $db_result = NULL;
	...
	// constructor
	function OldTransliteration($id_transliteration, $only_catalogue = false)
	{ // BEGIN constructor
		...
		if (! $only_catalogue) {
			...
			while($this->db_result->next_record()) {
				...
				while($na_radky->next_record()) {
					...
					$result[$surface['object_type'].'-'.$surface['surface_type'].'-id'] = $surface['id_surface'];
					$result[$surface['object_type'].'-'.$surface['surface_type'].'-line'][] = $na_radky->Record['transliteration'];
					$result[$surface['object_type'].'-'.$surface['surface_type'].'-line-id'][] = $na_radky->Record['id_line'];
