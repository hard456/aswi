...
**/
    function _generateLink ($title,$page=NULL,$escapeTitle,$pageDetails = NULL)
    {
		...
		if ($this->style=="ajax")
		{
			...
			if (isset($pageDetails['ajaxFormId']))
			{
				...
				$options['url'] = $url;
				$return .= $this->Ajax->Javascript->event("'$id'",
														"click",
    ...
    function renderFilter($array) {
    	...
    	$params = getUrlString($this->params['url']);
    	echo '<form id="FilterFormId" class="filtr" action="'.$this->Html->url('').$params.'" method="post">';
    		$firstCycle = true;
			...
			$options['with'] = "Form.serialize('FilterFormId')";
			$options['url'] = $this->_generateUrl(	$this->_pageDetails['page'],
														$this->_pageDetails);

			foreach ($array as $key => $value) {
				if ($firstCycle == false) echo '&nbsp;';

				echo $this->Html->selectTag('Filtr/'.$key,
											$value,
											$this->Html->tagValue('Filtr/'.$key),
											array(),
											array(),
											false);
				$id = 'Filtr' . Inflector::camelize($key);

				//pr($id);

				echo $this->Ajax->Javascript->event("'$id'", "change", $this->Ajax->remoteFunction($options));

				if ($firstCycle == true) $firstCycle = false;
			}

			//echo $this->Html->submit('Aplikuj filtr');
		echo '</form>';
    }
}
